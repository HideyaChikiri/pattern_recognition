...
def showFeatureSpace(X, W , row):
    ...
    for i in range(len(X)):
        if X[i][2] == 1:
            plt.scatter(X[i][1], 0, c = 'red', marker = "o")
        elif X[i][2] == 2:
            plt.scatter(X[i][1], 0, c = 'blue', marker = "s")
    ...
    while canContinue:
        ...
        for i in range(len(X)):
            ...
            if g_value < 0 and X[i][2] == 1:
                ...
                x = np.linspace(-2, 2)
                y = W[0] + x * W[1]
                plt.plot(x, y, c="red", linewidth=1.0, linestyle="-")
                canContinue = True
            elif g_value > 0 and X[i][2] == 2:
                print("2→1")
                W[0] = W[0] - row * X[i][0]
    ...
    x = np.linspace(-2, 2)
    y = W[0] + x * W[1]
    plt.plot(x, y, c="black", linewidth=1.0, linestyle="-")
